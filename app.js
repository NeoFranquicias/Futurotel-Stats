...
function renderRealTimePrices() {
  const pricesTbody = document.getElementById("real-time-prices-tbody");

  // Simulación de precios dinámicos
  fetch("hoteles.json")
    .then((response) => response.json())
    .then((hoteles) => {
      pricesTbody.innerHTML = ""; // Limpiar la tabla antes de renderizar

      hoteles.forEach((hotel) => {
        const precio = (Math.random() * 100 + 50).toFixed(2); // Precio aleatorio entre 50 y 150 €
...
setInterval(renderRealTimePrices, 10000);

// Renderizar precios al cargar la página
document.addEventListener("DOMContentLoaded", renderRealTimePrices);
...
function renderEventos() {
  ...
  eventos.forEach((ev, idx) => {
    ...
    tr.innerHTML = `
            <td><input type="checkbox" class="show-on-chart" data-id="${
              ev.id
            }" ${ev.visible ? "checked" : ""}></td>
            <td>${ev.fecha}</td>
            <td>${ev.evento}</td>
...
form.addEventListener("submit", function (e) {
  ...
  if (!demandaPorCiudad[ciudad]) {
    ...
    let arr = Array.from(
      { length: 30 },
      (_, i) => base + Math.floor(Math.sin(i / 4) * 4)
    );
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  function renderHotelMap(hoteles, kpiData) {
    ...
    hoteles.forEach((hotel) => {
      ...
      if (kpiData && kpiData.length) {
        ...
        if (kpi)
          ocup = Math.round(
            kpi.ocupacion.reduce((a, b) => a + b, 0) / kpi.ocupacion.length
          );
      }
      // Color por ocupación
      let color =
  ...
  fetch("hoteles.json")
    ...
    .then((data) => {
      ...
      kpiData = hoteles.map((hotel) => {
        ...
        let ocupacion = Array.from({ length: 30 }, (_, i) =>
          Math.round(baseOcup + Math.sin(i / 5) * 3 + Math.random() * 2)
        );
        let reservas = Array.from({ length: 30 }, (_, i) =>
          Math.round(ocupacion[i] * 2 + Math.random() * 10)
        );
        let cancel = Array.from({ length: 30 }, (_, i) =>
          Math.round(reservas[i] * Math.random() * 0.09)
        );
        let adr = Array.from({ length: 30 }, (_, i) =>
          Math.round(Math.random() * 30 + 90)
        );
      ...
      function renderRanking() {
        ...
        arr.forEach((h, i) => {
          let medalla = "";
          if (i === 0)
            medalla = '<span class="medalla oro" title="Oro">🥇</span>';
          else if (i === 1)
            medalla = '<span class="medalla plata" title="Plata">🥈</span>';
          else if (i === 2)
            medalla = '<span class="medalla bronce" title="Bronce">🥉</span>';
          tbody.innerHTML += `
                        <tr>
                            <td>${medalla || i + 1}</td>
                            <td>${h.nombre}</td>
      ...
      document
        ...
        .addEventListener("change", renderRanking);
      document
      ...
      let ingresosTotales = kpiData
        .flatMap((k) => k.ingresos)
        .reduce((a, b) => a + b, 0);
      let adrMedia = avg(kpiData.flatMap((k) => k.adr));
      let revparMedia = avg(kpiData.flatMap((k) => k.revpar));
      let reservasTotales = kpiData
        .flatMap((k) => k.reservas)
        .reduce((a, b) => a + b, 0);
      let cancelTotales = kpiData
        .flatMap((k) => k.cancel)
        .reduce((a, b) => a + b, 0);
      // Mostrar KPIs
      ...
      setTimeout(() => {
        spark(
          "spark-ocupacion",
          kpiData.flatMap((k) => k.ocupacion),
          "#00eaff"
        );
        spark(
          "spark-ingresos",
          kpiData.flatMap((k) => k.ingresos),
          "#0ff4f8"
        );
        spark(
          "spark-adr",
          kpiData.flatMap((k) => k.adr),
          "#00bcd4"
        );
        spark(
          "spark-revpar",
          kpiData.flatMap((k) => k.revpar),
          "#00e676"
        );
        spark(
          "spark-reservas",
          kpiData.flatMap((k) => k.reservas),
          "#ff851b"
        );
        spark(
          "spark-cancelaciones",
          kpiData.flatMap((k) => k.cancel),
          "#ff1744"
        );
      }, 0);
    });
  ...
  function renderCompareChart(kpiData, nombres) {
    ...
    let maxOcup = Math.max(
      ...kpiData.map((h) =>
        Math.round(h.ocupacion.reduce((a, b) => a + b, 0) / h.ocupacion.length)
      ),
      100
    );
    let maxIng = Math.max(
      ...kpiData.map((h) => h.ingresos.reduce((a, b) => a + b, 0)),
      1
    );
    let maxAdr = Math.max(
      ...kpiData.map((h) =>
        Math.round(h.adr.reduce((a, b) => a + b, 0) / h.adr.length)
      ),
      1
    );
    let maxRevpar = Math.max(
      ...kpiData.map((h) =>
        Math.round(h.revpar.reduce((a, b) => a + b, 0) / h.revpar.length)
      ),
      1
    );
    let maxSat = 100;
    const datasets = dataSel.map((h, i) => {
      let ocup =
        Math.round(
          h.ocupacion.reduce((a, b) => a + b, 0) / h.ocupacion.length
        ) || 0;
      let ing = h.ingresos.reduce((a, b) => a + b, 0) || 0;
      let adr =
        Math.round(h.adr.reduce((a, b) => a + b, 0) / h.adr.length) || 0;
      let revpar =
        Math.round(h.revpar.reduce((a, b) => a + b, 0) / h.revpar.length) || 0;
      ...
      options: {
          ...
          tooltip: {
            callbacks: {
              label: function (context) {
                ...
                const real =
                  context.dataset.realValues && context.dataset.realValues[i];
                let label = context.dataset.label || "";
                let metric = context.chart.data.labels[i];
                let value =
                  typeof real !== "undefined" ? real : context.parsed.r;
        ...
        animation: {
          ...
          onProgress: function (animation) {
            ...
            if (animation.currentStep === 1 && ctx)
